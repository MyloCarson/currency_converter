...
var _idb = __webpack_require__(1);
...
var dbPromise = _idb2.default.open('currency_converterDB', 1, function (upgradeDB) {
...
dbPromise.then(function (db) {
    ...
    console.log("[IDB] ", val);
    if (val.length < 1) getCountriesAndCurrencies();else {
        listOfCountries = val;
        addToSelects();
    }
});

var getCountriesAndCurrencies = function getCountriesAndCurrencies() {
    fetch(base_url + "/currencies").then(function (data) {
        data.json().then(function (e) {
            for (var country in e.results) {
...
var addToSelects = function addToSelects() {
    ...
    for (var _country in countries) {
        ...
        _item.value = countries[_country].id;
        if ('currencySymbol' in countries[_country]) {
            _item.text = countries[_country].currencyName + " => " + countries[_country].currencySymbol;
        } else {
            _item.text = "" + countries[_country].currencyName;
        }
        secondCurrency_Select.appendChild(_item);
    }
};

var convertCurrency = function convertCurrency(amount) {

    var firstCurrency = firstCurrency_Select.value;
    var secondCurrency = secondCurrency_Select.value;
    ...
    fetch(base_url + "/convert?q=" + firstCurrency + "_" + secondCurrency + "&compact=y").then(function (response) {
        response.json().then(function (data) {
            var res = data[indexInResponse].val * amount;
            resultDiv.innerText = listOfCountries[secondCurrency_Select.selectedIndex].currencySymbol + "  " + res;
        });
    }).catch(function (err) {
